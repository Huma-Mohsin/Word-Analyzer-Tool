...
const answer: {
  sentence: string;
} = //const answer: { sentence: string }: This line declares a constant named answer and specifies its type. In TypeScript, you can explicitly specify the type of a variable using a colon (:) followed by the type. Here, answer is declared to be an object with a property named sentence, which must be a string.
...
console.log(
  chalk.yellowBright(
    `Words In A Given Sentence: ${chalk.blueBright(words.length)}`
  )
);
//Author- Huma Mohsin
